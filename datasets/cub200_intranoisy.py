...
def build_set(root, year, train, noise_type='pairflip', noise_rate=0.5):
    """
        ...
        if train:
            with temp_seed(0):
                if noise_type == 'pairflip':
                    ...
                    noise_rate_per_class = noise_rate / (200-1)
                    noise_dist = np.ones(200) * noise_rate_per_class
                    noise_dist[0] = 1 - noise_rate
                    label_noises = np.random.multinomial(1, noise_dist, len(imgs)).argmax(1)

            imgs = [(x, (y + label_noises[i]) % 200) for i, (x, y) in enumerate(imgs)]
...
class CUB200(data.Dataset):
    ...
    def __init__(self, root, year, train=True, transform=None, target_transform=None, download=False,
                 loader=default_loader, noise_type='pairflip', noise_rate=0.45):
        ...
        self.imgs, self.classes, self.class_to_idx = build_set(os.path.join(self.root, self.raw_folder),
                                                               self.year, self.train, noise_type, noise_rate)
